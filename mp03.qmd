...
![NYC Parks logo](images/parks.png){.parks-logo}
...
# Sample trees just for performance
trees_sample <- trees |>
...
leaflet(options = leafletOptions(preferCanvas = TRUE)) |>
  ...
  # Title (centered)
  addControl(
    html = "<h3 style='text-align:center; margin:4px;'>Figure 2: Interactive Map for NYC Tree Locations</h3>",
    position = "topleft") |>
  ...
  # Tree locations as simple points
  addCircleMarkers(
    data = trees_sample,
    radius = 2,
...
invisible(district_most_trees)
```

- Council District **`{r} district_most_trees$CounDist`** has the most trees.
...
invisible(district_highest_density)
```
- Council District **`{r} district_highest_density$CounDist`** has the highest density of trees.
...
invisible(district_highest_dead_fraction)
```
- Council District **`{r} district_highest_dead_fraction$CounDist`** has the highest fraction of dead trees.
...
invisible(most_common_manhattan_species)
```

- The most common tree species in Manhattan is **`{r} most_common_manhattan_species$genusspecies`**.
...
```
- The tree closest to Baruch’s campus is a **`{r} closest_tree_species`**.
...
invisible(district26_shade_summary)
```
...
- District 26 currently has **`{r} district26_shade_summary$shade_trees`** shade trees.
Add 20% more, or **`{r} district26_shade_summary$shade_target_20pct`** new shade trees.
...
invisible(district26_stumps)
```
- Replace the **`{r} district26_stumps$n_stumps` stumps** in District 26 with new plantings.

- The altogether addition of **2114 new trees** (additional shade trees + replacement of stumps) would increase the **total tree count of `{r} district26_shade_summary$total_trees`** trees by **~14%**. 
...
p26 <- ggplot() +
  ...
  labs(
    title = "Figure 5: Shade Trees in District 26",
  )
...
p33 <- ggplot() +
  ...
  labs(
    title = "Figure 6: Shade Trees in District 33",
  )

p26 + p33 + plot_layout(widths = c(1, 1))
```
...
ggplot(
  ...
  theme_minimal() +
  labs(
    title    = "Figure 7: Stumps as a Share of All Tree Sites",
    x        = "Council District",
