...
## 1. Executive Summary
This report analyzes Netflix's data collection of its film and TV, with the goal of translating viewing patterns into clear, compelling stories that celebrate success, spotlight momentum, and frame opportunities for growthThe Public Relations (PR) team proposes 3 press releases:
...
The PR team acquired data from [Netflix's TuDum Top 10](https://www.netflix.com/tudum/top10), the official companion site that publishes data on Netflix's most popular shows and movies. The selected data sets include the Global Top 10 and Country-wide Top 10. They contain information 
...
- Netflix provides over 200 weeks of service history for all countries within the data set except **`{r} country_under_200$country_name`**. Netflix ceased operations there as of **`{r} country_under_200$last_week`**, which is the last week recorded.
...
red_notice_2021 <- GLOBAL_TOP_10 |>
  ...
  ungroup() |>
  mutate(runtime_hours = 118/60,                          # 1 hour 58 minutes
    approx_views_2021  = total_hours_2021 / runtime_hours) # hours ÷ hours-per-view
```
